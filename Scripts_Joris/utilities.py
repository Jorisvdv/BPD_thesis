...
def calculateMetrics(inner, X_test, y_test, fold=None):
    """
    ...
    fold (int, optional): The fold number of the outer loop. Used for the legend of the roc curve plot.
    ...
    if fold is not None:
        legend = f"ROC curve fold {fold}"
    else:
        legend = None
    roc_curve_plot = RocCurveDisplay.from_estimator(inner, X_test, y_test, name=legend)
    plt.close()  # Prevent plot from showing in notebook

    # Create calibration curve using CalibrationDisplay
    # Create legend for plot
    if fold is not None:
        legend = f"Calibration curve fold {fold}"
    else:
        legend = None
    calibration_curve_plot = CalibrationDisplay.from_estimator(
        inner, X_test, y_test, name=legend
    )
    plt.close()  # Prevent plot from showing in notebook
...
def nested_CV(
    ...
    Returns:
    models_and_scores (pandas.DataFrame): A dataframe containing the scores for each model.
    """

    # Set parameters to empty dict if not specified
    ...
    models_and_scores = pd.DataFrame(
        {
    ...
    for fold, (trainidx, testidx) in enumerate(outer_cv_folds.split(X, y)):
        ...
        models_and_scores.at[fold, "estimator"] = inner

        # Save features used in model in model scores
        models_and_scores.at[
            fold, "features"
        ] = X_train.columns.values  # inner.feature_names_in_

        # Calculate score metrics
        metrics = calculateMetrics(inner, X_test, y_test, fold=fold)

        # Calculate score metrics and save
        models_and_scores.at[fold, "test_accuracy"] = metrics["accuracy"]
        models_and_scores.at[fold, "test_roc_auc"] = metrics["roc_auc"]
        models_and_scores.at[fold, "test_f1"] = metrics["f1"]
        # Save roc curve in model scores
        models_and_scores.at[fold, "roc_curve"] = metrics["roc_curve"]
        # Save calibration curve in model scores
        models_and_scores.at[fold, "calibration_curve"] = metrics["calibration_curve"]
...
def export_model_and_scores(
    ...
    # Set datestamp first so it is the same for model and metrics
    ...
    # Select best model (selected on outer loop of nested cross validation, so on test set)
    best_model = models_and_scores["estimator"][
        models_and_scores["test_roc_auc"].argmax()
    ]
    ...
    if save_scores:
        # Export model to txt file as pretty json
        metrics_file_name = name_model + "_" + date_time

        # Create metrics dictionary to export
        metrics_dict = {
            "model_type": name_model,
            "selected_features": list(
                set(np.concatenate(models_and_scores["features"]).tolist())
            ),
            "best_model_parameters": best_model.best_estimator_.get_params(),
        }
        # Add mean and standard deviation of metrics to dictionary
        metrics_dict.update(
            models_and_scores.loc[:, "test_accuracy":"test_f1"]
            .describe()
            .loc[["mean", "std"]]
            .to_dict()
        )
        ...
        with open(
            metrics_folder_location / (metrics_file_name + ".json"),
            "w",
            encoding="utf-8",
        ) as f:
        ...
        create_roc_curve(models_and_scores, model_name=name_model).savefig(
            metrics_folder_location / (metrics_file_name + "_ROC" + ".png")
        )

        # Save calibration curve plot
        create_calibration_curve(models_and_scores, model_name=name_model).savefig(
